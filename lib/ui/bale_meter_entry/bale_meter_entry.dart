...
import 'package:classified_app/ui/bale_meter_entry/scan_meter_entry.dart';
...
import 'bloc/bale_meter_bloc.dart';

class BaleMeterEntry extends StatefulWidget {
  const BaleMeterEntry({Key? key}) : super(key: key);

  @override
  State<BaleMeterEntry> createState() => _BaleMeterEntryState();
}

class _BaleMeterEntryState extends State<BaleMeterEntry> {
  int pcs = 0;
  ...
  @override
  void initState() {
    pcs = BlocProvider.of<BalesListBloc>(context).selected.pcs ?? 0;
    _generate(pcs);
    _paginate(pgNo);
    super.initState();
  }
                    ...
                    ElevatedButton(
                        onPressed: () {
                          if (pgNo != lastPage) {
                            ...
                            log(entry.toString());
                            BlocProvider.of<BaleMeterBloc>(context).add(
                                GetToBaleSubmissionReviewPage(
...
class BaleMeterEntryWidget extends StatelessWidget {
  const BaleMeterEntryWidget({
    Key? key,
  ...
  @override
  Widget build(BuildContext context) {
    return BlocListener<BaleMeterBloc, BaleMeterState>(
      listener: (context, state) {
        if (state is BaleMeterEntryByBarcodeState) {
                    ...
                    onTap: () async {
                      final result = await Navigator.push(
                        ...
                        MaterialPageRoute<String>(
                          builder: (BuildContext context) => BaleScanMeterEntry(
                              index: index,
